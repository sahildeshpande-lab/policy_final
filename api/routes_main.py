...
async def health_check():
    ...
    try:
        ...
        vector_store = service_container.get(VectorStoreService)
    ...
    try:
        ...
        llm_service = service_container.get(LLMService)
...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        messages = await conv_service.get_messages(thread_id) if thread_id else []
        context = context_service.extract_context_from_messages(messages)
        
        # Check if this is a follow-up query
        is_follow_up = context_service.is_follow_up_query(req.message, context)
        
        # Detect intent from user message
        intent_result = intent_detector.process_message(req.message)
        
        # If it's a follow-up, adjust intent based on context
        if is_follow_up:
            contextual_intent, confidence = context_service.infer_intent_from_context(req.message, context)
            if confidence > intent_result['confidence']:
                intent_result['intent'] = contextual_intent
                intent_result['confidence'] = confidence
                
                # Get contextual response
                contextual_response = context_service.get_contextual_response(req.message, context)
                if contextual_response:
                    intent_result['response'] = contextual_response
        ...
        return ChatResponse(
            ...
            intent_response=intent_result["response"],
            is_follow_up=is_follow_up,
            context_topic=context.get('current_topic')
        )
